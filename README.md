...
### Telegram
Athena can use private Telegram bot-to-bot messages when paired with the [`mythic_telegram_profile`](https://github.com/DavidCarliez/mythic_telegram_profile) Mythic C2 profile. Each concurrently running payload requires a dedicated agent bot token, and Bot-to-Bot Communication Mode must be enabled for both the agent and controller bots.
